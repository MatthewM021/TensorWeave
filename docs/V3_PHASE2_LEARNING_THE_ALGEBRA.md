...
predicted.  The original production artifact is
`v3_recovery/PHASE2_ALGEBRA_POWER_CONTROL_V1.json`.  After the final source
freeze, the same control was regenerated byte-deterministically as
`v3_recovery/PHASE2_ALGEBRA_POWER_CONTROL_V2.json`; its file SHA-256 is
...
### V4 forward-only corrective replication: formal pass

V4 was frozen only after the V3 result was opened.  It therefore remains a
disclosed, nonconfirmatory corrective replication.  The sole instrument
change pads each case to the focal-query count per output class.  Every cell
now has 15 semantic cases, 96 total queries, and 24 focal queries; the added
queries are nonfocal balance controls, not new independent semantic evidence.
The semantic cases and focal queries are unchanged from V3.

The fresh two-block, 40-environment execution passed all preopen gates.  All
1,520 candidate fits and all 40 final models had zero TRAIN mistakes and zero
local overrides; the final serialized models independently replayed 18,858 /
18,858 TRAIN queries.  All 760 folds and all 40 environment aggregates tied
penalties 4 and 16 exactly.  The frozen stronger-penalty tie-break selected 16
in every environment, so validation did not empirically prefer that penalty.
The 1,520 candidate cleanliness and pseudoquery rows are typed and hash-bound
trusted-runner certificates because candidate models are not serialized; only
the 40 final models' direct-TRAIN fits are independently replayed.

The atomic batch open formally passed 40 / 40 environments.  The models
answered 3,840 / 3,840 actual-cell queries and 960 / 960 focal queries, plus
76,800 / 76,800 balanced rotated-cell queries and 19,200 / 19,200 rotated
focal queries.  Every environment recovered the same exact 20-entry table:
`BIND(x)=x`, `COPY(x)=x`, and `UPDATE_t(x)=x+t+1 mod 4` for supported
transforms `t in {0,1,2}`.  All 12 families, all three path relations, and
every per-cell inventory passed; each of the four shortcuts was strictly
worse both overall and focally in every environment.

The exact evidence is under `v3_recovery/phase2_outer_rotation_v4/`.
`SUMMARY.json` keeps the formal V4 pass separate from the immutable V3
failure.  This supports robust coefficient recovery inside the supplied
addressable-register representation.  It is not representation discovery,
secret-law discovery, or confirmatory evidence.
...
- fit coefficients on direct generator split `train` only;
- tune and select from the direct generator split `validation`, which also
  excludes the real held-out pair, after trusted-controller censoring;
- allow the trusted fold controller to use the task configuration, oracle
  occupancy, and dependency metadata only to certify whole-sequence censoring
  and descendant-query score masks; erase those fields before every estimator
  call;
- do not pass heldout masks or IDs, oracle routes, occupancy, generation IDs,
  dependency parents, or test seeds into the coefficient estimator;
- freeze hypothesis family, state dimension, rank, law weight, stopping rule,
  ...
  held-out pair;
- reject inconsistent or poisoned controller metadata before estimation, and
  require the pure estimator to be invariant to fields absent from its
  sanitized trace corpus; declared `train` query targets are the sole labels
  allowed in coefficient-estimator calls, while the trusted selector may use
  only the censored direct-`validation` query targets;
...
4. score causally selected descendant queries from the censored direct-
   `validation` trace corpus for that pseudoheldout cell;
...
8. only after the all-environment terminal gate, construct and open the
   balanced real heldout suite once.

Balanced inner direct/compositional probes remain a possible stronger future
variant; they were not used by the executed V3/V4 selector.
...
The original evaluation fixture begins with `BIND(0,0)` followed quickly by
...
observed-exception power control, frozen 40-environment V3 execution, and
forward-only 40-environment V4 corrective replication are now complete.  The
next sequence is:

1. preserve the failed V3 protocol and its exact behavioral audit without a
   post-hoc pass reclassification;
2. preserve the passing V4 corrective result without presenting its added
   nonfocal controls as new semantic evidence;
3. freeze a genuinely label-held prospective environment outside this already
   inspected task family;
4. extend the implemented rank-4-to-5 opaque zero-suffix diagnostic miniature
   into a full arbitrary-suffix predictive state/operator realization;
